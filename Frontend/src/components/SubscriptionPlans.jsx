...
const SubscriptionPlans = () => {
  ...
  const plans = [
    { name: "1 Month", price: 15, description: "1 Month subscription" },
  ...
  // Define the Flutterwave config inside the component
  const config = {
    public_key: 'FLWPUBK_TEST-30010dbc0f63942239a7796d2c3bea45-X', // Your public key
    tx_ref: Date.now(), // Transaction reference
    amount: selectedPlan ? selectedPlan.price : 0, // Amount in smallest currency unit (cents)
    currency: currency,
  ...
  // Flutterwave config with callback and closing the modal after response
  const fwConfig = {
    ...config,
    ...
    callback: (response) => {
      ...
      // Handle payment success (simulating successful payment)
      if (response.status === 'successful') {
        // For test mode, simulate success and redirect
        alert('Payment successful!');
        window.location.href = '/classroom'; // Redirect to the classroom or desired URL
      }
    },
    onClose: () => {
  ...
  // Flutterwave modal hook
  const { openPaymentModal } = useFlutterwave(fwConfig);
      ...
      <div className="container mx-auto px-4">
        ...
        {isFormVisible && (
          <div className="mt-8 p-6 bg-gray-800 rounded-lg">
            ...
            <div className="mt-6">
              <FlutterWaveButton
                {...fwConfig} // Pass config directly here instead of fwConfig
